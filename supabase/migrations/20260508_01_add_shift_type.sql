-- =====================================================
-- Migration: Add shift_type to rosters table
-- Date: 2026-05-08
-- =====================================================
-- Adds a shift_type column (lunch/dinner) to the rosters
-- table so staff can have separate lunch and dinner
-- scheduled shifts on the same day.
--
-- This enables the staff "Scheduled Shifts" view to group
-- by day with lunch/dinner distinction.
-- =====================================================

-- 1. Add shift_type column with default 'lunch' for existing rows
ALTER TABLE rosters ADD COLUMN shift_type TEXT NOT NULL DEFAULT 'lunch'
  CHECK (shift_type IN ('lunch', 'dinner'));

-- 2. Drop any existing unique constraint on (staff_id, roster_date)
--    and replace with one that includes shift_type
ALTER TABLE rosters DROP CONSTRAINT IF EXISTS unique_roster_staff_date;
CREATE UNIQUE INDEX unique_roster_staff_date_shift
  ON rosters(staff_id, roster_date, shift_type);
